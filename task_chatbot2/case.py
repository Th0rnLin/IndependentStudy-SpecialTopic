...
class Case:
  def __init__(self, ):
    ...
    f = open('database/disaster_case/抓動物.txt', 'r', encoding='utf-8')
    for i in f.readlines():
      self.catch_animals.append(i.replace('\n', ''))
    f.close()

    f = open('database/disaster_case/火災.txt', 'r', encoding='utf-8')
    for i in f.readlines():
      self.fire.append(i.replace('\n', ''))
    f.close()

    f = open('database/disaster_case/緊急救護.txt', 'r', encoding='utf-8')
    for i in f.readlines():
      self.first_aid.append(i.replace('\n', ''))
    f.close()

    f = open('database/disaster_case/受困.txt', 'r', encoding='utf-8')
    for i in f.readlines():
