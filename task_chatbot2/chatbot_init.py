...
cas = Case() # cas.predict(text) => output : '火災', '緊急救護', '抓動物', '受困'
...
def response():
  ...
  if not injured_flag:
    goal['injured_num'] = '0'
    return '請問有多少人需要救護車', 'injured'

#@title update_address_first_flag
...
def rorw_response():
  ...
  # response case (True or False)
    ...
    '''
    if goal['case'] == '火災': resp.append('請問發生的是火災嗎')
    elif goal['case'] == '緊急救護': resp.append('請問是有人受傷或生病嗎')
    elif goal['case'] == '抓動物': resp.append('請問是有動物需要抓捕嗎')
    elif goal['case'] == '受困': resp.append('請問是有人受困嗎')
    '''
  # response number of injured (True or False)
    ...
    if goal['injured_num'] == '0':
      ...
      resp.append('請問是' + str(goal['injured_num']) + '人需要救護車嗎')
