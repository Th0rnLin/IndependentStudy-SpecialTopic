  ...
  if mode == 'run':
    ...
    def rorw_response(text):
      ...
      # response case (True or False)
        ...
        keyword, _ = cas.get_aid_keyword(text)
        case_resp = ['請問發生的是火災嗎', '請問是' + keyword + '嗎', '請問是有動物需要抓捕嗎', '請問是有人受困嗎']
      ...
      # response number of injured (True or False)
        ...
        if goal['case'][0] in ['火災', '受困'] and goal['injured_num'] is '0':##
          ...
          if goal['injured_num'] == '0':
            ...
            resp.append('我們預計派出' + str(goal['injured_num']) + '輛救護車')
    ...
    while(True):
      ...
      counter += 1
    try:
      print('final goal =>address: {}\n\t     case: {}\n\t     number of injured: {}'.format(''.join(list(goal['address'].values())), goal['case'][0], goal['injured_num']))
    except:
      print(goal)
